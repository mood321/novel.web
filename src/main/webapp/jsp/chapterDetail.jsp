		...
		<c:if test="${isSuccess }">
			...
			<!-- 前一章 章节列表 后一章 链接区域 -->
				<div style="text-align: center;">
					<a
						href="./chapterDetail.do?url=${chapterDetail.prev }&beasUrl=${beasUrl}">前一张</a>
					<a href="./chapterList.do?url=${beasUrl }">章节列表</a> <a
						href="./chapterDetail.do?url=${chapterDetail.next }&beasUrl=${beasUrl}">后一张</a>
				</div>
			</c:if>
		<c:if test="${!isSuccess }">
			很抱歉,读取失败，请重新读取
